...
#define WEB_SERVER "116.62.81.138"
#define WEB_PORT "80"
...
char request[] = "GET https://api.seniverse.com/v3/weather/now.json?key=pikf7iubxfzrr1kw&location=ip&language=zh-Hans&unit=c\n\n";
char recv_buf[500];
...
//			WEB_URL, WEB_SERVER, strlen(request_body), request_body);
		...
		server_addr.sin_addr.s_addr = inet_addr("116.62.81.138");
		connect(connect_socket, (struct sockaddr*)&server_addr, sizeof(server_addr));
		
		send(connect_socket, request, strlen(request), 0);
		recv(connect_socket, recv_buf, sizeof(recv_buf) - 1, 0);
...
void button_setup() {
	...
	gpio_isr_handler_add(34, button_isr_handler, NULL);	
}
